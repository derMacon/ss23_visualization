...
def weekday_importance(df):
    ...
    fig = plt.figure()
    axes = fig.add_axes([0.1, 0.1, 0.68, 0.75])
    axes.bar(count,mo, color = 'r')
    axes.bar(count,di,bottom=mo)
    axes.bar(count, mi, bottom= mi_bot)
    axes.bar(count, do, bottom=do_bot)
    axes.bar(count, fr, bottom=fr_bot)
    axes.bar(count, sa, bottom=sa_bot)
    axes.bar(count, so, bottom=so_bot)
    axes.set_xlabel("decade")

    axes.set_ylabel("attendance per weekday in %")
    axes.legend(["Monday", "Tuesday", "Wednesday","Thursday","Friday","Saturday","Sunday"],bbox_to_anchor=(1.02, 1),loc = 'upper left')
    axes.set_title("What Weekday is the most popular?")
    fig.savefig('week1', bbox_inches='tight')
    plt.show()
